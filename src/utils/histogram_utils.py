...
def histogram_lane_detection(img, steps, search_window, h_window):
    ...
    masked_img = img[:, search_window[0]:search_window[1]]  # y pixels between two x-coordinates
    pixels_per_step = img.shape[0] // steps  # number of pixels each step will the windows will take
    ...
    for i in range(steps):
        ...
        histogram_smooth = signal.medfilt(histogram, h_window)  # normalize the histogram
...
def highest_n_peaks(histogram, peaks, n=2, threshold=0):
    ...
    :param histogram: A histogram data structure
    :param peaks: list of peak indexes
    :param n: amount of peaks to select
    :param threshold: minimum value the peaks must be

    :return: N highest peaks of a histogram above a given threshold
    """
